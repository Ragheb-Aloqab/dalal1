<x-app-layout-admin>
    <x-slot name="header">
        <x-admin.title text="تعديل صورة منزلقة" />
            ...
            <form action="{{ route('admin.sliders.update', $slider->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="relative">
                    <input type="text" id="title" name="title"
                        ...
                        value="{{ old('title', $slider->title) }}" required aria-describedby="title-error">
                    @error('title')
                ...
                <!-- Image Upload -->
                <label for="path" class="block mb-2 text-sm font-medium text-dark dark:text-white">تحميل
                    الصورة</label>
                <div class="relative">
                    <input type="file" id="path" name="path"
                        ...
                        accept="image/*" aria-describedby="path-error">
                    @error('path')
                        <div class="absolute inset-y-0 flex items-center pointer-events-none end-0 pe-3">
                            <i class="text-lg font-medium leading-tight ti ti-alert-circle text-error"></i>
                        </div>
                ...
                <div class="relative mb-2">
                    <textarea id="description" name="description"
                        ...
                        rows="4" aria-describedby="description-error">{{ old('description', $slider->description) }}</textarea>
                    @error('description')
                        <div class="absolute inset-y-0 flex items-center pointer-events-none end-0 pe-3">
                            <i class="text-lg font-medium leading-tight ti ti-alert-circle text-error"></i>
                        </div>
                ...
                <div class="flex justify-end gap-3 mt-6">
                    <button class="btn btn-md btn-primary" type="submit">تحديث الصورة</button>

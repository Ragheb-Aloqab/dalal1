<x-app-layout-admin>
    <x-slot name="header">
        <x-admin.title text="المزودين" />
        <x-admin.breadcrumb :items="[
            ['url' => route('admin.dashboard.index'), 'icon' => 'ti ti-home', 'label' => 'الصفحة الرئيسية'],
            ['label' => 'المزودين'],
        ]" />
    </x-slot>
    <x-admin.search-and-add searchUrl="{{ route('admin.providers.index') }}"
        createUrl="{{ route('admin.providers.create') }}" />
        ...
        <x-admin.table-body>
            @foreach ($users as $user)
                <tr class="search-items">
                    ...
                    <x-admin.table-data>
                        <x-admin.table-action-buttons :viewUrl="route('admin.providers.show', $user->id)" :editUrl="route('admin.providers.edit', $user->id)" :deleteUrl="route('admin.providers.destroy', $user->id)" />
                    </x-admin.table-data>
                </tr>
            @endforeach
        </x-admin.table-body>

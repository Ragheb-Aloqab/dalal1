<x-app-layout-admin>
    <x-slot name="header">
        <x-admin.title text="العملاء" />
        <x-admin.breadcrumb :items="[
            ['url' => route('admin.dashboard.index'), 'icon' => 'ti ti-home', 'label' => 'الصفحة الرئيسية'],
            ['label' => 'العملاء'],
        ]" />
    </x-slot>
    <x-admin.search-and-add searchUrl="{{ route('admin.clients.index') }}"
        ...
        <x-admin.table-body>
            @foreach ($users as $user)
                <tr class="search-items">
                    ...
                    <x-admin.table-data>
                        <x-admin.table-action-buttons :viewUrl="route('admin.clients.show', $user->id)" :editUrl="route('admin.clients.edit', $user->id)" :deleteUrl="route('admin.clients.destroy', $user->id)" />
                    </x-admin.table-data>
                </tr>
            @endforeach
        </x-admin.table-body>
